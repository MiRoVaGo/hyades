...
prec <- foreach(idx = 1:length(filenames), .combine = rbind) %dopar% {
  ...
  dummie <- unique(dummie[, .(id = dummie_name, na_yrs = sum(na_yrs),
                              start = min(YEAR), end = max(YEAR), record)])
  return(dummie)
}

p00 <- ggplot(prec, aes(x = na_yrs)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 10, center = 5,
                 fill = "#FFC20A", color = "gray23") +
  #geom_line(aes(y = dbeta(nas, 1.1, 8)), linewidth = 2) +
  scale_x_continuous(breaks = seq(0, 160, 20)) +
  labs(x = "Number of years with missing values",
...
ggsave(plot = p00, "figures/yrs_missing.png", width = 4.5*GOLDEN_RATIO,
       height = 4.5)
